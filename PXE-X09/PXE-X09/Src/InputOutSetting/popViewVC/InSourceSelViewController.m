...
#import "SourceItem.h"
#define SourceItemTag 20
@interface InSourceSelViewController ()
@property(nonatomic,strong)UILabel *chTopLabel;
@property(nonatomic,strong)NSArray *NameArray;
@property(nonatomic,strong)NSArray *ImageArray;
@end

@implementation InSourceSelViewController
{
    int input_source_temp;
}
- (void)viewDidLoad {
    [super viewDidLoad];
   input_source_temp=RecStructData.System.input_source;
    ...
    self.chTopLabel.text=[LANG DPLocalizedString:@"L_InputSourceSelect"];
    ...
    for (int i=0; i<5; i++) {
        ...
        item.selectBlock = ^(BOOL isSelected, NSInteger itemTag) {
            ...
            if (value==0) {
                if (RecStructData.System.mixer_source !=1) {
                   input_source_temp=value;
                }
            }else if(value==1){
                if (RecStructData.System.mixer_source!=0) {
                   input_source_temp=value;
                }
            }else{
               input_source_temp=value;
...
-(void)okClick{
    RecStructData.System.input_source=input_source_temp;
    self.mixerBlock();
    [self dismissView];
}
-(void)flashItem{
