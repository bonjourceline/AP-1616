...
//音量条类型
typedef NS_ENUM(NSUInteger, SeekBarIndexPStyle) {
    SeekBarIndexP_LINE_ONLY = 1,  //只有圆形线
    SeekBarIndexP_LINE_PARK = 2,  //只有圆形线，线是分段的，像时钟那样
    SeekBarIndexP_LINE_IMAGE = 3, //有线和图片
    SeekBarIndexP_LINE_PARK_IMAGE = 4, //有圆形线，线是分段的，像时钟那样和图片
    SeekBarIndexP_LINE_IMAGE_INDEX_P = 5, //点图片和图片
};


@interface SeekBarIndexP : UIControl{
    @private
    double WIND_Width;
    double WIND_Height;
    double WIND_CenterX;
...
@property (nonatomic) SeekBarIndexPStyle SBStyle; //控件样式,默
