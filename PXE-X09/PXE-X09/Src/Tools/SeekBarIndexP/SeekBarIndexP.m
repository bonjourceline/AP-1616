...
#import "SeekBarIndexP.h"
#import <math.h>


#define P_ArcRadius 0.9
#define P_ImgRadius 0.98
...
- (void)setup{
    ...
    self.SBStyle = SeekBarIndexP_LINE_IMAGE_INDEX_P;//VCSS_LINE_ONLY VCSS_LINE_PARK VCSS_LINE_IMAGE VCSS_LINE_PARK_IMAGE
    Progress = 170;//当前数值
    ...
    if(self.SBStyle >= SeekBarIndexP_LINE_IMAGE){
        ...
        self.MasterImg.image = [UIImage imageNamed:[PIMG DPIMG:@"chs_sb_button_bg"]];
...
- (void)drawRect:(CGRect)rect {
    ...
    /*画中心圆形按键*/
    if(self.SBStyle == SeekBarIndexP_LINE_IMAGE){////有线和图片
        CGContextRef context = UIGraphicsGetCurrentContext();
        ...
        CGContextDrawPath(context, kCGPathStroke);
    }else if(self.SBStyle == SeekBarIndexP_LINE_ONLY){//只有圆形线，线是分段的，像时钟那样
        CGContextRef context = UIGraphicsGetCurrentContext();
        /*画seekbar弧BGProgress*/
        CGContextSetStrokeColorWithColor(context, ColorArc_BGProgress.CGColor);
        CGContextSetLineWidth(context, ProgressWidth);
        ...
        CGContextDrawPath(context, kCGPathStroke);
    }else if(self.SBStyle == SeekBarIndexP_LINE_PARK){//只有圆形线，线是分段的，像时钟那样
        //画Progress Bg
    ...
    }else if(self.SBStyle == SeekBarIndexP_LINE_IMAGE_INDEX_P){//只有圆形线，线是分段的，像时钟那样
        CGAffineTransform transform= CGAffineTransformMakeRotation(sweepAngle);//*0.38
        self.MasterImg.transform = transform;
...
-(void)drawIndexPhoto{
    ...
    double Step = SB_AngleMax/19;
    for(int i=0;i<19;i++){
        CGPoint Point = [self pointFromAngle:(180+SB_AngleStart + Step*i+Step/2) withRadius:T_Radius_LineOut];
        CGRect imageRect = CGRectMake(Point.x-[Dimens GDimens:30]/2,
                                      Point.y-[Dimens GDimens:30]/2,
                                      [Dimens GDimens:30],
                                      [Dimens GDimens:30]);
        
        if(SB_AngleStep*(MaxProgress-Progress) > Step*i){
            CGContextDrawImage(context, imageRect, [UIImage imageNamed:[PIMG DPIMG:@"chs_master_val_normal"]].CGImage);
        }else{
            CGContextDrawImage(context, imageRect, [UIImage imageNamed:[PIMG DPIMG:@"chs_master_val_press"]].CGImage);
        }
    }
    CGContextDrawPath(context, kCGPathStroke);
}
