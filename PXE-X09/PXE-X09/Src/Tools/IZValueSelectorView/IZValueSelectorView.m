...
- (NSString *)selectedImageName
{
    if (!_selectedImageName) {
        _selectedImageName = [PIMG DPIMG:@"selectorRect"];
    }
...
- (void)createContentTableView {
    ...
    //selectionImageView.frame.size.height = [Dimens GDimens:ChannelBtnHeight];
    ...
    if (self.horizontalScrolling) {
        ...
        CGRect tempRect = selectionImageView.frame;
        tempRect.size.height = [Dimens GDimens:ChannelBtnListHeight];//_contentTableView.rowHeight; //调整文字高度 选中
        selectionImageView.frame = tempRect;
    }
    else {
    ...
    //[_contentTableView setBackgroundColor:[UIColor colorWithPatternImage:[UIImage imageNamed:[self selectedImageName]]]];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([contentSubviews count] >0 ) {
        ...
        contentSubV.backgroundColor = SetColor(UI_Channel_BtnListBg);// [UIColor colorWithRed:16/255.0 green:26/255.0 blue:74/255.0 alpha:1.0]; //add by H.Q.Z
        ...
        viewToAdd.backgroundColor = SetColor(UI_Channel_BtnListBg);
        [UIColor colorWithRed:16/255.0 green:26/255.0 blue:74/255.0 alpha:1.0];//add by H.Q.Z
