...
- (void)setup{
    ...
    self.SBStyle = EQSB_Gain_TLINE_IMAGE;//VCSS_LINE_ONLY VCSS_LINE_PARK VCSS_LINE_IMAGE VCSS_LINE_PARK_IMAGE
    Progress = 170;//当前数值
    ...
    if(self.SBStyle >= EQSB_Gain_LINE_IMAGE){
        ...
        self.MasterImg.image = [UIImage imageNamed:[PIMG DPIMG:@"toning_sb_val_bg"]];
...
- (void)drawRect:(CGRect)rect {
    ...
    /*画中心圆形按键*/
    if(self.SBStyle == EQSB_Gain_LINE_IMAGE){////有线和图片
        CGContextRef context = UIGraphicsGetCurrentContext();
        ...
        CGContextDrawPath(context, kCGPathStroke);
    }else if(self.SBStyle == EQSB_Gain_LINE_ONLY){//只有圆形线，线是分段的，像时钟那样
        CGContextRef context = UIGraphicsGetCurrentContext();
        /*画seekbar弧BGProgress*/
        CGContextSetStrokeColorWithColor(context, ColorArc_BGProgress.CGColor);
        CGContextSetLineWidth(context, ProgressWidth);
        ...
        CGContextDrawPath(context, kCGPathStroke);
    }else if(self.SBStyle == EQSB_Gain_LINE_PARK){//只有圆形线，线是分段的，像时钟那样
        //画Progress Bg
    ...
    }else if(self.SBStyle == EQSB_Gain_TLINE_IMAGE){////有线和图片
        CGContextRef context = UIGraphicsGetCurrentContext();
        CGAffineTransform transform= CGAffineTransformMakeRotation(sweepAngle);//*0.38
        self.MasterImg.transform = transform;
