//
//  QIZAllEffectTableViewController.m
//  ReNetTuning
//
...
@interface QIZAllEffectTableViewController ()<LocalEffectTableViewCellDelegate,UITableViewDataSource,UITableViewDelegate,UIDocumentInteractionControllerDelegate,UIAlertViewDelegate>
{
...
@implementation QIZAllEffectTableViewController
...
- (void)viewDidLoad {
    ...
    [self.bottomView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.size.mas_equalTo(CGSizeMake( KScreenWidth,kBottomViewH+1));
    }];
...
-(void)doMultiCollectionBtn:(UIButton *)sender
{
    ...
    NSLog(@"self.arrayM.count=%ld",self.arrayM.count);
    for (int i=0; i<self.arrayM.count; i++) {
        LocalEffectTableViewCell *cell = [self.tableView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:i inSection:0]];
         SEFFFile *effData = (SEFFFile *)self.arrayM[i];
...
-(void)doShareEffect:(UIButton *)sender
{
    ...
    NSLog(@"doShareEffect fileName = %@",fileName);
    NSLog(@"doShareEffect filePath = %@",filePath);
...
-(void)doMultiCheckBtn:(UIButton *)sender
{
    ...
    self.multiSelectLabel.text = selectedStr;
    [self.arrayM replaceObjectAtIndex:self.selectIndex withObject:effData];
    [self.tableView reloadData];
