...
#define P_ArcRadius 0.9
#define P_ImgRadius 0.90

#define T_RadiusOut 0.80
#define T_RadiusIN  0.69
#define T_RadiusCircle  0.65
#define ToRad(deg)         ( (M_PI * (deg)) / 180.0 )
#define ToDeg(rad)        ( (180.0 * (rad)) / M_PI )
#define SQR(x)            ( (x) * (x) )

@implementation VolumeCircleIMLine
...
- (void)setup{
    ...
    T_Radius_Circle  = WIND_MRidius * T_RadiusCircle;
    ...
    ColorCirInside = SetColor(UI_Master_SB_Volume_BG);//seekbar 条底颜色
    
    
    if((self.SBStyle == VCSS_LINE_IMAGE)||(self.SBStyle == VCSS_LINE_PARK_IMAGE)||(self.SBStyle==VCSS_LINE_OnlyImage)){
        self.MasterImg = [[UIImageView alloc]init];
        self.MasterImg.frame = CGRectMake(WIND_MRidius * (1-P_ImgRadius),
                                          WIND_MRidius * (1-P_ImgRadius), ImgRadius*2, ImgRadius*2);
        self.MasterImg.image = [UIImage imageNamed:@"master_vol_indicator"];
        [self addSubview:self.MasterImg];
        if (self.SBStyle==VCSS_LINE_OnlyImage) {
            ColorArc_Progress = [UIColor clearColor];//seekbar 进度颜色
            ColorArc_BGProgress = [UIColor clearColor];//seekbar 条底颜色
            ColorProgressPoint = [UIColor clearColor];//seekbar 条底颜色
            ColorCirInside = [UIColor clearColor];//seekbar 条底颜色
        }
        /*
         self.MasterImgBg = [[UIImageView alloc]init];
         self.MasterImgBg.frame = CGRectMake(WIND_MRidius * (1-P_ImgRadius),                                          WIND_MRidius * (1-P_ImgRadius), ImgRadius*2, ImgRadius*2);
         self.MasterImgBg.image = [UIImage imageNamed:@"master_vol_indicator_bg"];
         [self addSubview:self.MasterImgBg];
         */
    }else if(self.SBStyle == VCSS_LINE_Thumb){
...
- (void)drawRect:(CGRect)rect {
    ...
    /*画中心圆形按键*/
    if(self.SBStyle == VCSS_LINE_IMAGE){////有线和图片
        CGContextRef context = UIGraphicsGetCurrentContext();
        ...
        CGContextDrawPath(context, kCGPathStroke);
    }else if(self.SBStyle == VCSS_LINE_ONLY){//只有圆形线，线是分段的，像时钟那样
        CGContextRef context = UIGraphicsGetCurrentContext();
        /*画seekbar弧BGProgress*/
        CGContextSetStrokeColorWithColor(context, ColorArc_BGProgress.CGColor);
        CGContextSetLineWidth(context, ProgressWidth);
        ...
        CGContextDrawPath(context, kCGPathStroke);
    }else if(self.SBStyle == VCSS_LINE_PARK){//只有圆形线，线是分段的，像时钟那样
        //画Progress Bg
    ...
    }else if(self.SBStyle == VCSS_LINE_PARK_IMAGE){//只有圆形线，线是分段的，像时钟那样
        ...
        //画点
        //[self draw_T_ProgressPoint];
    }else if(self.SBStyle == VCSS_LINE_J){
        CGContextRef context = UIGraphicsGetCurrentContext();
        /*画seekbar弧BGProgress*/
        CGContextSetStrokeColorWithColor(context, ColorArc_BGProgress.CGColor);
        CGContextSetLineWidth(context, ProgressWidth);
        ...
        CGContextSetStrokeColorWithColor(context, ColorArc_BGProgress.CGColor);
        CGContextSetLineWidth(context, ProgressWidth);
        CGContextAddArc(context, WIND_CenterX, WIND_CenterY, ArcRadius,  0, 2*M_PI, false);//0,2*M_PI Draw_AngleEnd Draw_AngleStart
        CGContextDrawPath(context, kCGPathStroke);
        
        //        填充圆，无边框
        CGContextSetFillColorWithColor(context, ColorCirInside.CGColor);//填充颜色
        CGContextAddArc(context, WIND_CenterX, WIND_CenterY, ArcRadius-ProgressWidth/2, 0, 2*M_PI, 0); //添加一个圆
