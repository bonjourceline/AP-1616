...
- (void)setup
{
    ...
    if (_popUpViewAlwaysOn)
    {
        [self _showPopUpViewAnimated:NO];
    }
...
- (void)updatePopUpView
{
    ...
    if ((valueString = [self.dataSource slider:self stringForValue:self.value]) && valueString.length != 0) {
        ...
        valueString = self.showValue;//[_numberFormatter stringFromNumber:@(self.showValue)];
        popUpViewSize = [self calculatePopUpViewSize];
    }
    popUpViewSize.width += 20;
    ...
    //valueString = [valueString substringToIndex:valueString.length-3];
